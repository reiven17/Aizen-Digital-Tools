# Assets – Aizen Digital Tools

Esta carpeta contiene todos los recursos gráficos del sitio.  
Sube los archivos en la carpeta correspondiente respetando los nombres exactos indicados.

---

## 📁 `iconos/`

Iconos de redes sociales y navegador usados en el sitio.

| Archivo            | Uso                          |
|--------------------|------------------------------|
| `tiktok.png`       | Botón de red social (footer) |
| `facebook.png`     | Botón de red social (footer) |
| `whatsapp.png`     | Botón de red social (footer) |
| `favicon-32.png`   | Favicon del navegador        |
| `apple-touch-icon.png` | Acceso directo iOS      |

**Tamaño recomendado:** 64 × 64 px o 128 × 128 px, fondo transparente (PNG).

---

## 📁 `logos/`

Logotipos de la marca usados en loader, hero, navbar y footer.

| Archivo               | Uso                                              |
|-----------------------|--------------------------------------------------|
| `logo-pantalla-carga.png` | Loader (pantalla de carga)                  |
| `aizen-logo.png`      | Logo principal web (hero)                         |
| `aizen-logo-sm.png`   | Navbar y Footer (versión compacta)               |
| `og-image.png`        | Imagen Open Graph para redes sociales (meta tag) |

**Tamaños recomendados:**
- `logo-pantalla-carga.png` → ancho ~420 px, fondo transparente.
- `aizen-logo.png` → ancho ~300 px, fondo transparente.
- `aizen-logo-sm.png` → ancho ~120 px, fondo transparente.
- `og-image.png` → **1200 × 630 px** (proporción 1.91:1, estándar Open Graph).

---

## 📁 `metodos-pago/`

Métodos de pago usados en la sección **Confianza**.

| Archivo                | Uso                                      |
|------------------------|------------------------------------------|
| `binance.png`      | Chip de método de pago       |
| `paypal.png`       | Chip de método de pago       |
| `visa.png`         | Chip de método de pago       |
| `yape.png`         | Chip de método de pago       |

---

## 📁 `productos/`

Imágenes de portada de cada producto del catálogo (usadas en tarjetas, carousel y modal).

| Archivo                  | Producto                     |
|--------------------------|------------------------------|
| `antigravity.png`        | Antigravity Ultra            |
| `gemini.png`             | Gemini Ultra                 |
| `gemini-pro.png`         | Gemini Pro                   |
| `gemini-workspace.png`   | Gemini Ultra Workspace       |
| `grok.png`               | SuperGrok                    |
| `grok-heavy.png`         | SuperGrok Heavy              |
| `capcut.png`             | CapCut Pro                   |
| `chatgpt.png`            | ChatGPT Plus                 |
| `chatgpt-business.png`   | ChatGPT Business             |
| `adobe.png`              | Adobe Pro Plus               |
| `n8n.png`                | N8n Starter                  |
| `canva.png`              | Canva Pro                    |
| `claude.png`             | Claude Pro                   |
| `claude-max.png`         | Claude Max 5x                |

**Tamaño recomendado:** 400 × 400 px, fondo transparente o degradado de marca (PNG).

---

## Próximos pasos

1. Exporta cada imagen con el nombre exacto indicado en las tablas.
2. Sube los archivos a la carpeta correspondiente (puedes arrastrarlos directamente en GitHub o usar `git add`).
3. Los archivos `.gitkeep` en cada carpeta pueden eliminarse una vez que se suban imágenes reales.
4. Verifica que el sitio cargue correctamente abriendo `index.html` en el navegador.
